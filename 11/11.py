...
for y, row in enumerate(data):
    for x, number in enumerate(row):
        products = [largest_prod]
        # check the following row, column, and diagonal
        # row
        products.append(numpy.prod([number] + row[x+1:x+4]))
        # column
        if y+3 < len(data):
            products.append(numpy.prod([number, data[y+1][x], data[y+2][x], data[y+3][x]]))
...
print("largest product:", largest_prod)
